...
const getPrompt = (topic) =>
  fs.readFileSync(`prompts/${topic}.txt`, { encoding: "utf8", flag: "r" });
const getQuestion = (msg) => msg.content.replace(`${COMMAND_PREFIX} `, "");
const generateEmbed = (title, description, probability) =>
  new MessageEmbed()
    .setTitle(title.substring(0, 256))
    .setColor("#d24ede")
    .setDescription(description)
    .setFooter({
      text: `Answer generated using GPT-3 | Model: ${GPT_MODEL} | Confidence: ${
        Math.round(probability * 100) ?? "N/A"
      }%`,
    });
...
const extraProbability = (httpResp) =>
  Math.E **
  Object.values(httpResp.data.choices[0].logprobs.top_logprobs[0]).sort()[0];
...
async function determineQuestionCategory(question) {
  ...
  */
  const choice = extractResponse(
    await GPT3(`${SORT_QUESTIONS_PROMPT + question} ->`)
  );
  ...
  if (choice === "This question is about whether an image is allowed") {
    return "allowed";
  }
  if (choice === "This question is a general inquiry") {
    return "general";
  }
  return "off-topic";
}
...
client.on("messageCreate", async (msg) => {
  ...
  */
  if (
    ...
    const category = await determineQuestionCategory(question);
    if (category === "off-topic") {
      // No off-topic questions!
      const embed = generateEmbed(
        "This Bot is for Questions Related to DALL-E",
        "At this time, I am only taking questions about DALL-E 2. Your question appears to be off-topic."
      );
      msg.channel.send({ embeds: [embed] });
    } else {
      const prompt = `${promptMap[category] + question}\nA:`;

      // TODO: Additional prompt iteration in playground using "Show Probabilities"
      const GPTResponse = await GPT3(prompt);
      const response = extractResponse(GPTResponse);
      const probability = extraProbability(GPTResponse);
